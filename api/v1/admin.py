...
async def create_invite(
    invite: InviteCreate, current_user: User = Depends(get_current_admin_user)
):
...
async def deactivate_invite(
    invite_id: int, current_user: User = Depends(get_current_admin_user)
):
...
async def get_workspace_users(
    workspace_id: int, current_user: User = Depends(get_current_admin_user)
):
...
async def get_workspace_chats(
    offset: int = 0, current_user: User = Depends(get_current_admin_user)
):
...
async def update_preferences(
    preferences: SystemPreferences, current_user: User = Depends(get_current_admin_user)
):
