...
"""
### Chat
# - `POST /api/v1/chat/settings`: Alter chat settings
# - `POST /api/v1/chat/send`: Send message
# - `POST /api/v1/chat/select/ai`: Select AI
# - `POST /api/v1/chat/select/knowledge_graph`: Select Knowledge Graph
# - `GET /api/v1/chat/history/{sessionId}`: Get chat history for a session
# - `POST /api/v1/chat/history/{sessionId}`: Save chat history for a session
"""
...
from beta.data.obj import Message
        ...
        for i, row in enumerate(self.df.collect()):
            if row["id"] == message_id:
                ...
                updated_msg.edit_history.append(
                    {"timestamp": datetime.utcnow(), "edit_note": update.edit_note}
                )
