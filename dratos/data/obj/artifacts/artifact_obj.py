...
import lancedb  # Added to resolve undefined name 'LanceDB'
...
class Artifact:
    ...
    schema: ClassVar[daft.Schema] = daft.Schema.from_pyarrow_schema(artifact_schema)
    ...
    def __init__(
        ...
        if df is None:
            ...
            if files:
                self.df = self._load_files(files, uri_prefix, self.io_config)
        else:
            self.add_files(files, uri_prefix, self.io_config)

        self.df = self.validate_schema(self.df)

    def get_artifact_uri_prefix(
        self,
